...
FIRESTORE (dedicated collections — never share one with another feature)
  ...
  roas_sim_config/config            USER-EDITABLE config that used to live in the
                                    ...
                                    GP2 multipliers, the incrementality factors /
                                    share floors+caps, and the actuals-calibration knobs
                                    (enabled / priorStrength / minRatio / maxRatio).
                                    seed_config() creates it with the
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    # Simulator calibration: the dashboard scales each campaign's simulated curve by
    # measured ÷ simulated conversion value at the same spend (its Actuals row over the
    # simulation's own window, click-time first), clamps the ratio to [minRatio, maxRatio]
    # and shrinks it toward 1.0 by conversion volume (λ = n / (n + priorStrength)) before
    # reading any recommendation off it. The math runs client-side in
    # babyshop-roas-simulations.html (resolveCalibration); these knobs just ride the payload.
    "calibration": {
        "enabled": True,
        "priorStrength": 20,
        "minRatio": 0.5,
...
def _firestore():
    ...
    Because it IS memoised, the local fallback has to refresh: a bare gcloud token
    lasts about an hour, and this client outlives that by the whole length of a
    local dev session. See voyado_sync._credentials for the full write-up.
    """
    global _db_client
...
def _clean_config(raw: dict | None) -> dict:
    ...
    if isinstance(cal, dict):
        # enabled: forgiving bool. An unreadable value keeps the default rather than
        # silently switching calibration off (or on).
        if "enabled" in cal:
            v = cal.get("enabled")
            if isinstance(v, bool):
                out["calibration"]["enabled"] = v
            else:
                s = str(v).strip().lower()
                if s in ("1", "true", "yes", "on"):
                    out["calibration"]["enabled"] = True
                elif s in ("0", "false", "no", "off"):
                    out["calibration"]["enabled"] = False
        # priorStrength is a conversion COUNT, not a fraction — _metric only, no /100.
        ps = _metric(cal.get("priorStrength"))
        if isinstance(ps, (int, float)) and ps >= 0:
            out["calibration"]["priorStrength"] = float(ps)
        # Ratios are plain multipliers (0.5 = curve halved); must be positive. The
        # dashboard swaps a mis-entered min > max, so no ordering is enforced here.
        for key in ("minRatio", "maxRatio"):
            n = _metric(cal.get(key))
            if isinstance(n, (int, float)) and n > 0:
                out["calibration"][key] = float(n)
    return out

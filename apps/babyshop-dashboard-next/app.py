...
def api_kv_data(_: str = Depends(verify)):
    # Parallel run: computed from the Bluebird marts on demand (read-through
    # Firestore cache), no snapshot refresh job involved.
    import marts_source

    try:
        return marts_source.get_kv_payload()
...
def api_product_data(_: str = Depends(verify)):
    ...
    try:
        return marts_source.get_product_payload()
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=503)

...
(function () {
  ...
  /* ── Parallel-run watermark ─────────────────────────────────────────────
   * This deployment is the NEW dashboard reading the Bluebird warehouse
   * (claude-private-499703). The badge tells everyone at a glance which of
   * the two parallel dashboards they are looking at; remove it when the old
   * stack is sunset. Fixed-position so it survives any page's own layout. */
  function renderWatermark() {
    if (document.getElementById('parallel-run-badge')) return;
    var b = document.createElement('div');
    b.id = 'parallel-run-badge';
    b.setAttribute('title', 'Parallel run — data from claude-private-499703 (Bluebird marts). The old dashboard remains the reference until sign-off.');
    b.textContent = 'NY DATAKÄLLA · PARALLELLKÖRNING';
    b.style.cssText = [
      'position:fixed', 'right:14px', 'bottom:14px', 'z-index:99999',
      'font:600 11px/1 system-ui,sans-serif', 'letter-spacing:.08em',
      'padding:7px 12px', 'border-radius:999px',
      'background:#0E6B61', 'color:#fff',
      'box-shadow:0 2px 8px rgba(0,0,0,.25)', 'pointer-events:auto',
      'user-select:none'
    ].join(';');
    (document.body || document.documentElement).appendChild(b);
  }
  if (document.body) renderWatermark();
  else document.addEventListener('DOMContentLoaded', renderWatermark);
